...
public class Monobank
{
    ...
    public async Task<IEnumerable<CurrencyInfo>> GetCurrencyRatesAsync()
    {
        ...
        return code switch
        {
            200 => JsonToObject<IEnumerable<CurrencyInfo>>(body),
    ...
    public async Task<UserInfo> GetUserInfoAsync()
    {
        ...
        return code switch
        {
            200 => JsonToObject<UserInfo>(body),
    ...
    public async Task<IEnumerable<StatementItem>> GetStatementAsync(string account, DateTime from, DateTime to)
    {
        ...
        return code switch
        {
            200 => JsonToObject<IEnumerable<StatementItem>>(body),
            403 => throw new InvalidTokenException(),
            _ => throw new NotSupportedException()
        };
    ...
    /// <summary>
    /// Creates the invoice TODO continue...
    /// </summary>
    ...
    /// <summary>
    /// Gets the status of the invoice by its id. Designed to use when something went wrong with sync from merchant
    /// side or when there is no webhook set to receive the status update.
    /// </summary>
    /// <param name="invoiceId">The identifier of the invoice.</param>
    /// <returns>The <see cref="InvoiceStatus"/> object with detailed information about the invoice status.</returns>
    /// <exception cref="InvalidInvoiceIdException">thrown whenever provided <see cref="invoiceId"/> is null or empty string.</exception>
    /// <exception cref="InvalidTokenException">thrown when token used to authenticate the request is invalid.</exception>
    /// <exception cref="NotSupportedException">thrown if the received response has no supported method (please contact me if that happen)</exception>
    public async Task<InvoiceStatus> GetInvoiceStatusAsync(string invoiceId)
    {
        ...
        return code switch
        {
            200 => JsonToObject<InvoiceStatus>(body),
            400 => throw new InvalidRequestParameterException(JsonToObject<Error>(body)),
            403 => throw new InvalidTokenException(),
            _ => throw new NotSupportedException()
        };
    ...
    public async Task GetInvoicePaymentInfo(string invoiceId)
    {
        throw new NotImplementedException();
    }
    
    public async Task SplitPayment(string invoiceId, SplitItem[] splitList)
    {
        throw new NotImplementedException();
    }
    
    // TODO Add other methods from here: https://api.monobank.ua/docs/acquiring.html#/paths/~1api~1merchant~1invoice~1create/post
